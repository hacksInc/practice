 ...
 *  Ethna_DB_ADOdb(author: halt feits <halt.feits@gmail.com>)を基に作成
 *  @package    Ethna
 *  @access     public
 */
class Pp_DB_ADOdb extends Ethna_DB
{
    /**#@+
     *  @access private
     */
     ...
     */
    function Pp_DB_ADOdb(&$controller, $dsn, $persistent)
    {
...
//$all = $this->db->getAll("SHOW VARIABLES LIKE 'char%'");
//error_log('DEBUG:' . __FILE__ . ':' . __LINE__ . ':' . var_export($all, true));

		    $c =& Ethna_Controller::getInstance();
		    $config =& $c->getConfig();
			if ($config->get('is_test_site')) {
				$this->db->debug = 1;
				// ↑注意：trueではなく1にすること。
				// 　　　　adodb.inc.php(1071): if ($this->debug == 99) adodb_backtrace(true,5);
				// 　　　　が作動しないようにする為。
			}
