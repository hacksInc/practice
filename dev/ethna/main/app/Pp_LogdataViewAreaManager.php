...
/**
 *  Pp_LogDataViewAreaManager
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_LogdataViewAreaManager extends Pp_LogdataViewManager
{
	private $_csv_area_log_data = array(
		'id',
		'api_transaction_id',
		'processing_type',
		'pp_id',
	 ...
	 * @return mixed
	 */
	public function getAreaLogDataCount($search_params)
	{
		$conditions = $this->_getAreaLogDataConditions($search_params);
		$res = $this->_getAreaLogDataCount($conditions['where'], $conditions['param']);
		if ($res === false){
			return false;
		}
	 ...
	 * @return mixed
	 */
	public function getAreaLogData($search_params, $limit=null, $offset=null)
	{
		...
		$res = $this->_getAreaLogData($conditions['where'], $conditions['param'], $sort, $limit, $offset);
		if ($res === false){
			return false;
		}
	...
	/**
	 * エリアストレス履歴を取得する検索条件を編集する
	 *
	 * @parami array $search_params
	 * @return mixed
	 */
	private function _getAreaLogDataConditions($search_params)
	{
	 ...
	 * @return mixed
	 */
	private function _getAreaLogData($where, $param, $sort=null, $limit=null, $offset=null)
	{
		...
		$sql = "SELECT"
			...
			. " status_prev,"
			. " date_created"
			. " FROM log_area " . $where . $order_by . $option;
		// 実行
	 ...
	 * @return mixed
	 */
	private function _getAreaLogDataCount($where, $param)
	{
		$log_db = $this->backend->getDB('logex');
		$sql = "SELECT count(*) FROM log_area " . $where;
	 ...
	 * @return array
	 */
	public function getAreaDataByApiTransactionId($api_transaction_id)
	{
		$conditions = $this->_getApiTransactionIdConditions($api_transaction_id);
		$sort = array(
			'api_transaction_id' => 'ASC',
		);
		$res = $this->_getAreaLogData($conditions['where'], $conditions['param'], $sort);
		if ($res === false){
			return false;
		}
	 ...
	 * csvファイル作成を行う
	 *
	 * @pramas array $area_log_data
	 * @return mixed $file_name 正常時
	 *               Ethna_Error エラー時
	 */
	public function createCsvFileAreaLogData($area_log_data)
	{
		$log_name = 'area_log_data';
		$file_path = LOGDATA_PATH_AREA_DATA;

		return $this->createCsvFile($file_path, $log_name, $area_log_data, $this->_csv_area_log_data);
