<?php
/**
 *  Pp_LogDataViewStressManager.php
 *
 *  @author     {$author}
 *  @package    Pp
 *  @version    $Id$
...
/**
 *  Pp_LogDataViewStressManager
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_LogdataViewStressManager extends Pp_LogdataViewManager
{
	private $_csv_stress_log_data = array(
		'id',
		'api_transaction_id',
		'processing_type',
		'pp_id',
	 ...
	 * @return mixed
	 */
	public function getStressLogDataCount($search_params)
	{
		$conditions = $this->_getStressLogDataConditions($search_params);
		$res = $this->_getStressLogDataCount($conditions['where'], $conditions['param']);
		if ($res === false){
			return false;
		}
	 ...
	 * @return mixed
	 */
	public function getStressLogData($search_params, $limit=null, $offset=null)
	{
		...
		$res = $this->_getStressLogData($conditions['where'], $conditions['param'], $sort, $limit, $offset);
		if ($res === false){
			return false;
		}
	...
	/**
	 * ストレスケア履歴を取得する検索条件を編集する
	 *
	 * @parami array $search_params
	 * @return mixed
	 */
	private function _getStressLogDataConditions($search_params)
	{
		...
		// A02 臨時ストレスケア
		// A05 セラピー診断
		$where = $where . " AND processing_type in ('A01', 'A02', 'A05')";
	 ...
	 * @return mixed
	 */
	private function _getStressLogData($where, $param, $sort=null, $limit=null, $offset=null)
	{
	 ...
	 * @return mixed
	 */
	private function _getStressLogDataCount($where, $param)
	{
		$log_db = $this->backend->getDB('logex');
	...
	/**
	 * api_transaction_idを元にストレスケア履歴情報を取得する
	 *
	 * @param mixed $api_transaction_id
	 * @return array
	 */
	public function getStressDataByApiTransactionId($api_transaction_id)
	{
		$conditions = $this->_getApiTransactionIdConditions($api_transaction_id);
		$sort = array(
			'api_transaction_id' => 'ASC',
		);
		$res = $this->_getStressLogData($conditions['where'], $conditions['param'], $sort);
		if ($res === false){
			return false;
		}
	 ...
	 * csvファイル作成を行う
	 *
	 * @pramas array $stress_log_data
	 * @return mixed $file_name 正常時
	 *               Ethna_Error エラー時
	 */
	public function createCsvFileStressLogData($stress_log_data)
	{
		$log_name = 'stress_log_data';
		$file_path = LOGDATA_PATH_STRESS_DATA;

		return $this->createCsvFile($file_path, $log_name, $stress_log_data, $this->_csv_stress_log_data);
