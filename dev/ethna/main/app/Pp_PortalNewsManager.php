...
class Pp_PortalNewsManager extends Ethna_AppManager
{
	...
	function getPickup ()
	{
		...
		$sql =	"SELECT m.id AS news_id, m.* FROM m_portal_news m " .
				"WHERE m.news_kbn = ? AND m.del_flg = ? AND CAST(m.open_date AS DATE) <= NOW() " .
				"ORDER BY m.regist_date DESC LIMIT 1";
	...
	function getNews ( $news_id )
	{
		...
		$sql =	"SELECT * FROM m_portal_news " .
				"WHERE ID = ? AND news_kbn = ? AND del_flg = ? AND CAST(open_date AS DATE) <= NOW()";
