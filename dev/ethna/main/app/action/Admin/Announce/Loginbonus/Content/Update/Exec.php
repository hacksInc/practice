<?php
/**
 *  Admin/Announce/Loginbonus/Content/Update/Exec.php
 *
 *  @author     {$author}
 *  @package    Pp
 *  @version    $Id$
 ...
 *  @package    Pp
 */
class Pp_Form_AdminAnnounceLoginbonusContentUpdateExec extends Pp_AdminActionForm
{
    /**
     *  @access private
...
/**
 *  admin_announce_loginbonus_content_update_exec action implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_Action_AdminAnnounceLoginbonusContentUpdateExec extends Pp_AdminActionClass
{
    /**
     *  preprocess of admin_announce_loginbonus_content_update_exec Action.
     *
     *  @access public
...
*/
    ...
    function perform()
    {
		...
		foreach($data as $key => $val) {
			...
			$ret = $loginbonus_m->updateLoginBonusItem($columns);
			if (!$ret || Ethna::isError($ret)) {
				if ($transaction) $db->rollback();
				$this->af->setAppNe('err_msg', $ret);
		        return 'admin_announce_loginbonus_content_error';
