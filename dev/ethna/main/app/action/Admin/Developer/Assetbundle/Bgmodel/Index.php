...
/** admin_developer_assetbundle_bgmodel_* で共通のアクションフォーム定義 */
class Pp_Form_AdminDeveloperAssetbundleBgmodel extends Pp_Form_AdminDeveloperAssetbundle
{
	/**
	...
	function checkAssetbundleFile($name)
	{
		...
		// 先頭を確認
		if (!preg_match('/^Dungeon_/', $joint_file_name) &&
		    !preg_match('/^FieldMap_/', $joint_file_name)
		) {
		...
		if (!preg_match('/^[0-9]+$/', $bgmodel_id_tmp)) {
			$this->ae->add($name, $name . "のファイル名が不正です。", E_ERROR_DEFAULT);
			return;
		}
...
/**
 *  admin_developer_assetbundle_bgmodel_index Form implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_Form_AdminDeveloperAssetbundleBgmodelIndex extends Pp_Form_AdminDeveloperAssetbundleBgmodel
{
    /**
     *  @access private
...
/**
 *  admin_developer_assetbundle_bgmodel_index action implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_Action_AdminDeveloperAssetbundleBgmodelIndex extends Pp_AdminActionClass
{
    /**
     *  preprocess of admin_developer_assetbundle_bgmodel_index Action.
     *
     *  @access public
...
*/
    ...
    function perform()
    {
        return 'admin_developer_assetbundle_bgmodel_index';
    }
}
