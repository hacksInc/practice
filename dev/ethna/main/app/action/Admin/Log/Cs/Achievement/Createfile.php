<?php
/**
 *  Admin/Log/Cs/Achievement/Createfile.php
 *
 *  @author     {$author}
 *  @package    Pp
 *  @version    $Id$
...
/** admin_log_cs_achievement_* で共通のアクションフォーム定義 */
	/*class Pp_Form_AdminLogCsAchievementCreatefile extends Pp_Form_AdminLogCsAchievement
	 {
	}*/

/**
 *  admin_log_cs_achievement_createfile Form implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
...
/**
 *  admin_log_cs_achievement_createfile action implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_Action_AdminLogCsAchievementCreatefile extends Pp_AdminActionClass
{
	const MAX_PAGE_DATA_COUNT = '100';
	const MAX_DATA_COUNT = '10000';

	/**
	 *  preprocess of admin_log_cs_achievement_createfile Action.
	 *
	 *  @access public
	...
	function perform()
	{
		...
		if ($achievement_log_count == 0) {
			$this->af->setApp('code', 400);
			$this->af->setApp('err_msg', "対象となるデータが存在しません。");
			return 'admin_log_cs_json_encrypt';
		}

		$achievement_log_data = $logdata_view_m->getAchievementLogData($search_params);
		$res = $logdata_view_m->createCsvFileAchievementLogData($achievement_log_data['data']);
		if ($res === false){
			$this->af->setApp('code', 400);
