...
class Pp_Form_AdminLogCsMonsterPowerup extends Pp_Form_AdminLogCsMonster
{
}

/**
 *  admin_log_cs_monster_powerup_index Form implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_Form_AdminLogCsMonsterPowerupIndex extends Pp_Form_AdminLogCsMonster
{
    /**
     *  @access private
...
/**
 *  admin_log_cs_monster_powerup_index action implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_Action_AdminLogCsMonsterPowerupIndex extends Pp_AdminActionClass
{
    const MAX_PAGE_DATA_COUNT = '100';
    const MAX_DATA_COUNT = '10000';

    /**
     *  preprocess of admin_log_cs_monster_powerup_index Action.
     *
     *  @access public
    ...
    function perform()
    {
        ...
        if ($search_flg != '1'){
                return 'admin_log_cs_monster_powerup_index';
        }

        $monster_powerup_log_count = $logdata_view_m->getMonsterPowerupLogDataCount($search_params);
        if ($monster_powerup_log_count > $data_max_cnt) {
            $this->af->setApp('monster_log_count', -1);
            return 'admin_log_cs_monster_powerup_index';
        }
        if ($monster_powerup_log_count <= 0) {
            $this->af->setApp('monster_log_count', 0);
            return 'admin_log_cs_monster_powerup_index';
        }
        $monster_powerup_log_data = $logdata_view_m->getMonsterPowerupLogData($search_params, $limit, $offset);

        // api_transaction_idでひも付くモンスターログ情報を取得する(モンスター単位での売却価格を取得)
        foreach ($monster_powerup_log_data['data'] as $k => $v) {
            $transaction_id_list[] = $v['api_transaction_id'];
        }

        // 売却時取得アイテム情報
        $powerup_item_list = $logdata_view_item_m->getItemDataByApiTransactionId($transaction_id_list);
        $item_data_list = '';
        foreach($powerup_item_list['data'] as $k => $v){
            $item_data_list[$v['api_transaction_id']] = $v;
        }

        $pager = $logdata_view_m->getPager($monster_powerup_log_count, $offset, $limit);

        $this->af->setApp('monster_powerup_log_list', $monster_powerup_log_data['data']);
        $this->af->setApp('monster_powerup_log_count', $monster_powerup_log_count);
