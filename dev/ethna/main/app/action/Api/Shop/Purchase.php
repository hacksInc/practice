...
error_log( "[Purchase]update user base Error" );
			...
			$columns = array(
				...
				'num' => ((( empty( $item )) ? 0 : $item['num'] ) + $this->m_sell['num'] ),
...
error_log( "[Purchase]log accounting Error" );
			...
			// KPIはここで処理
			$uym = date( "ym", strtotime( $user['date_created'] ) );
			if ( $ua == 1 ) {
				$kpi_m->log( "Apple-ppp-mt_" . $uym . "_install_user_sales", 4, 1, time(), $pp_id, $this->m_sell['price'], "", "" );
			} else {
				$kpi_m->log( "Google-ppp-mt_" . $uym . "_install_user_sales", 4, 1, time(), $pp_id, $this->m_sell['price'], "", "" );
			}
		}
