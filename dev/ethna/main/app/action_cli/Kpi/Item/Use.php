 ...
 *  @package    Pp
 */
class Pp_Cli_Form_KpiItemUse extends Pp_ActionForm
{
    /**
     *  @access private
 ...
 *  @package    Pp
 */
class Pp_Cli_Action_KpiItemUse extends Pp_CliActionClass
{
    ...
    function perform()
    {
        ...
        $res = $kpiview_m->getKpiItemUseDataByDateItemTallyCount($tran_date);
        ...
        $res = $kpiview_m->getLogItemDataForKpiItemUse($param);
        if($res === false){
            exit(1);
        }

        // データの集計を行いDBに登録
        $res = $kpiview_m->editLogItemDataforKpiItemUseCount ($res['data'], $tran_date);
        if ($res === false){
            exit(1);
        }
