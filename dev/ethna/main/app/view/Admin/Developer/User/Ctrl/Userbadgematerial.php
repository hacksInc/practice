...
/**
 *  admin_developer_user_ctrl_userbadgematerial view implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_View_AdminDeveloperUserCtrlUserbadgematerial extends Pp_AdminViewClass
{
    /**
    ...
    function preforward()
    {
		...
		$badge_master = $badge_m->getMasterBadgeMaterial();
		$badge_list = $badge_m->getUserBadgeMaterialList($user_id);
		
		$user_badge = array();
		foreach($badge_list as $key => $val) {
			$material_id = $val['material_id'];
			foreach($badge_master as $mkey => $mval) {
				if ($mval['material_id'] == $material_id) {
...
//		$list = $developer_m->getMasterList($table);
		...
		if (!empty($list)) {
			foreach ($list as $row) {
				...
				$values['no'] = $idx++;
				foreach($badge_master as $mkey => $mval) {
					if ($mval['material_id'] == $values['material_id']) {
	...
	//error_log("metadata=".print_r($metadata,true));
		...
		$metadata2[] = $metadata[1];//material_id
		$metadata2[] = array(
			'name'     => 'name',
			'label'    => 'バッジ素材名',
			'datatype' => 'string',
