...
/**
 *  admin_log_cs_monster_powerup_info view implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Pp
 */
class Pp_View_AdminLogCsMonsterPowerupInfo extends Pp_AdminViewClass
{
    ...
    function preforward()
    {
        ...
        // モンスター強化合成情報
        $monster_powerup_log_data = $logdata_view_m->getMonsterPowerupDataByApiTransactionId($api_transaction_id);
        ...
        // 増減アイテム情報
        $powerup_item_list = $logdata_view_item_m->getItemDataByApiTransactionId($api_transaction_id);

        // 勲章付与情報
        $achievement_list = $logdata_view_achievement_m->getAchievementDataByApiTransactionId($api_transaction_id);

        $this->af->setApp('monster_powerup_log', $monster_powerup_log_data['data'][0]);
        $this->af->setApp('monster_powerup_log_count', $monster_powerup_log_data['count']);
        $this->af->setApp('item_log_list', $powerup_item_list['data'][0]);
