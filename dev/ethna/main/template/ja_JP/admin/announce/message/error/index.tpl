        ...
        <div class="span9">
            ...
            <div class="dialog-content-box" style="margin-bottom:5px;">
                ...
                <form action="create/input" method="post">
                    <input type="submit" value="追加" class="btn btn-primary"> 新規エラーメッセージを追加します。
                </form>
                </div>
					...
					<div class="dialog-content-box row-fluid">
						<div style="width: 650px;">
                            <div class="row-fluid" style="margin: 10px;">
                                <dl class="dl-horizontal" style="margin: 3px;">
                                    <dt style="width:60px;">{form_name name="error_id"}</dt>
                                    <dd style="margin-left:80px;">{$row.error_id}</dd>
                                </dl>
                            </div>
							<div class="row-fluid" style="margin:10px;">
								{$row.message}
                        ...
                        <div class="row-fluid">
                            <div class="dialog-content-box" style="width: 200px;">
                                ...
                                <div style="margin:5px;">
                                    <input type="button" value="削除" class="btn btn-danger" id="btnDeleteMessage{$row.error_id}" name="btn_delete_message{$row.error_id}">
                                </div>
                                <div style="margin:5px;">
                                    <form action="create/input" method="post">
                                        <input type="hidden" name="error_id" value="{$row.error_id}">
                            ...
                            <div style="width: 200px;">
                                <div style="margin:10px;">
                                    {form_name name="date_created"}
                                    {$row.date_created}
                                </div>
                                <div style="margin:10px;">
                                    {form_name name="date_modified"}
                                    {$row.date_modified}
                                </div>
                            </div>
                        </div>
...
<div id="deleteConfirmDialog">
	...
	<form action="delete/exec" method="post" id="formDeleteMessageDialog">
        <input type="hidden" name="error_id" value="" id="deleteErrorId">
	</form>
</div>
...
<script>
    ...
    $("input[name^='btn_delete_message']").click(function(){
      var name = $(this).attr('name'),
          error_id = "";
      error_id = name.replace('btn_delete_message', '');
      $("#deleteErrorId").val(error_id);
