        ...
        <div class="span9">
            ...
            <div class="dialog-content-box" style="margin-bottom:5px;">
                ...
                <form action="create/input" method="post">
                    <input type="submit" value="追加" class="btn btn-primary"> 新規ヘルプバーメッセージを追加します。
                </form>
                </div>
						...
						<div style="width: 650px;">
                            <div class="row-fluid" style="margin: 10px;">
                                <dl class="dl-horizontal" style="margin: 3px;">
                                    <dt style="width:60px;">{form_name name="helpbar_id"}</dt>
                                    <dd style="margin-left:80px;">{$row.helpbar_id}</dd>
                                </dl>
                            </div>
                            <div class="row-fluid" style="margin:10px;">
                                {$row.message}
                            ...
                            <div class="dialog-content-box" style="width: 200px;">
                                <div style="margin:5px;">
                                    <form action="update/input" method="post">
                                        <input type="hidden" name="helpbar_id" value="{$row.helpbar_id}">
                                        <input type="hidden" name="btn_action" value="update">
                                        <input type="submit" value="修正" class="btn btn-info">
                                    </form>
                                </div>
                                <div style="margin:5px;">
                                    <input type="button" value="削除" class="btn btn-danger" id="btnDeleteMessage{$row.helpbar_id}" name="btn_delete_message{$row.helpbar_id}">
                                </div>
                                <div style="margin:5px;">
                                    <form action="create/input" method="post">
                                        <input type="hidden" name="helpbar_id" value="{$row.helpbar_id}">
...
<div id="deleteConfirmHelpbar">
    削除します。よろしいでしょうか？</ br>
	<form action="delete/exec" method="post" id="formDeleteMessageHelpbar">
        <input type="hidden" name="helpbar_id" value="" id="deleteHelpbarId">
	</form>
</div>
...
<script>
    ...
    $("input[name^='btn_delete_message']").click(function(){
      var name = $(this).attr('name'),
          helpbar_id = "";
      helpbar_id = name.replace('btn_delete_message', '');
      $("#deleteHelpbarId").val(helpbar_id);
      $('#deleteConfirmHelpbar').dialog('open');
    });

    $('#deleteConfirmHelpbar').dialog({
      autoOpen: false,
      title: 'ヘルプバーメッセージ削除確認',
      width: "300px",
      closeOnEscape: false,
      modal: true,
/*      position: {
      ...
      buttons: {
        "削除する": function(){
          $(this).dialog('close');
          $("#formDeleteMessageHelpbar").submit();
        },
        "キャンセル": function(){
          $(this).dialog('close');
        }
