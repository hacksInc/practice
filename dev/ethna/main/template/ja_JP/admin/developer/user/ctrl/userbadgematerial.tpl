...
<div class="container-fluid">
	...
	<div class="row-fluid">
		...
		<div class="span9">
			...
			<br />
			<form action="userbadgematerialaddsel" method="post">
				バッジ素材ID（改行区切）<br />
				<textarea name="material_list" cols=10 rows=3 wrap="hard"></textarea><br />
				<input type="hidden" name="id" value="{$app.base.user_id}" />
				<input type="hidden" name="table" value="{$form.table}" />
				<input type="submit" name="submit" value="指定バッジ素材を追加" class="btn end-btn2" />
			</form>
...
<script>
	window.onload = function() {
		...
		$('input.end-btn2').click(function() {
			return window.confirm('指定バッジ素材を追加しますがよろしいですか？');
		});
