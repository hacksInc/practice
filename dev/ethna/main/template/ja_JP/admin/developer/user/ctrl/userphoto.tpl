		...
		<div class="span6">
			...
			<tbody>
				...
				<tr>
					...
					<td>{$v.photo_id}</td>
					<td>{$v._photo_id}</td>
					<td><input type="text" data-key="{$k}" data-no="{$k+1}" id="edit1-photo_lv-{$k+1}" name="photo_lv" value="{$v.photo_lv}"></td>
	...
	$(document).ready(function(){
		...
		var UserView = Backbone.View.extend({
			...
			updateOnChange: function(e) {
				...
				attrs.id = attrs.pp_id + ',' + attrs.photo_id;

				var message = '<div role="alert" class="alert alert-success"><strong>Success！</strong>No.' + $(e.target).data('no') + 'のフォトレベルを変更しました。</div>'
