<!DOCTYPE html>
<html lang="ja">
{include file="admin/common/head.tpl" title="KPI アイテム ガチャ毎課金UU - サイコパス管理ページ"}
<body>
    ...
    <div class="row-fluid">
        ...
        <div {*class="span9 main-contents"*}>
            ...
            <div class="">
                ...
                {* 検索結果出力 *}
                {if $app.kpi_gacha_uu_count==0}
                <div class="alert alert-warning">
                    <span>対象となる集計結果は存在しません</span>
                </div>
                {else}
                    <div class="">
                        <span style="font-weight:bold;">{$app.kpi_date_from}</span>～<span style="font-weight:bold;">{$app.kpi_date_to}</span>のガチャ毎 課金UU
                    </div>
                <div>
                    <div class="kpi-list-float-box kpi-list-title-box">
                        ...
                        <div class="kpi-list-title content-list-part-320">
                            ...
                            <div class="kpi-list-float-box">
                                <div class="kpi-list-sub-title-gold content-list-part-80">有料ﾒﾀﾞﾙUU</div>
                                <div class="kpi-list-sub-title-gold content-list-part-80">無料ﾒﾀﾞﾙUU</div>
                                <div class="kpi-list-sub-title-gold content-list-part-80">総ﾒﾀﾞﾙUU</div>
                                <div class="kpi-list-sub-title-gold content-list-part-80">課金UU率</div>
                            </div>
                        </div>
                        {/foreach}
                    </div>
                    {foreach from=$app.kpi_gacha_uu_list key="key" item="uu_data"}
                    <div class="kpi-list-content-box">
                       <div class="kpi-list-float-box">
                            <div class="kpi-list-content content-list-part-70">
                                {$uu_data.date_gacha_tally}<br />
                                ({$uu_data.date_gacha_tally_day})
                            </div>
                            {*{foreach from=$uu_data.list key="list_key" item="list_data"}
                            <div class="kpi-list-content content-list-part-80">{$list_data.uu_item_pay}</div>
                            <div class="kpi-list-content content-list-part-80">{$list_data.uu_item_free}</div>
                            <div class="kpi-list-content content-list-part-80">{$list_data.uu_item_total}</div>
                            <div class="kpi-list-content content-list-part-80">{$list_data.gacha_uu_rate}%</div>
                            {/foreach}*}
	                        {foreach from=$app.kpi_gacha_name_list key="name_key" item="name_data"}
								{if isset($uu_data.list.$name_key)}
								    <div class="kpi-list-content content-list-part-80">{$uu_data.list.$name_key.uu_item_pay}</div>
						            <div class="kpi-list-content content-list-part-80">{$uu_data.list.$name_key.uu_item_free}</div>
				                    <div class="kpi-list-content content-list-part-80">{$uu_data.list.$name_key.uu_item_total}</div>
		                            <div class="kpi-list-content content-list-part-80">{$uu_data.list.$name_key.gacha_uu_rate}%</div>
								{else}
