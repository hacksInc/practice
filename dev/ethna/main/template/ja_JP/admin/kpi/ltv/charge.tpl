<!DOCTYPE html>
<html lang="ja">
{include file="admin/common/head.tpl" title="KPI 登録月別課金額&課金UU - サイコパス管理ページ"}
...
<div class="container-fluid">
	...
	<div class="row-fluid">
		...
		<div class="span10 main-contents">
			<div class="page-header"><h2>登録月別課金額&課金UU</h2></div>
			...
			<form action="charge" method="post" class="form-horizontal" id="formLogSearch">
				...
				<div class="button-wrap">
					...
					<input type="hidden" value="c" id="type" name="type">
					...
					<thead>
						<tr style="background-color:#EEEEEE">
							...
							{foreach from=$app.disp_elapsed_date key="key" item="kpi_list_title"}
								{if $kpi_list_title != 0}
									<th>{math equation="index + add_date_index" index=$kpi_list_title add_date_index="1"}ヵ月</th>
								{/if}
							{/foreach}
						</tr>
					</thead>
					...
					{foreach from=$app.list key="key" item="ltv"}
						<tr>
							...
							{foreach from=$ltv.list key="key2" item="ltv_info"}
								{if $ltv_info.rate!= '-'}
									<td>{$ltv_info.rate|string_format:"%.2f"}</td>
								{else}
									<td>{$ltv_info.rate}</td>
								{/if}
							{/foreach}
