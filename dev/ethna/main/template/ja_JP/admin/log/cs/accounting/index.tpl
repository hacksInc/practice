...
.search-list-content { height: 52px; }
</style>
{/literal}
	<body>
		...
		<div class="container-fluid">
			...
			<div class="row-fluid">
				...
				<div class="span9 main-contents">
					...
					{if $app.search_flg=="1"}
					{* 検索結果出力 *}
					{if $app.accounting_log_count==0}
					<div class="alert alert-warning">
						<span>対象となるログは存在しません</span>
					</div>
					{elseif $app.accounting_log_count==-1}
					...
					<div style="padding:5px;">
						<div class="search-list-title-box">
							<div>
								<span>{$app.accounting_log_count}件のログが抽出されました</span>
							</div>
					...
					<div>
						<div class="search-list-title-box">
							<div class="search-list-title content-list-part-170">購入日時</div>
							<div class="search-list-title content-list-part-150">ニックネーム</div>
							<div class="search-list-title content-list-part-150">ユーザーID</div>
							<div class="search-list-title content-list-part-100">購入アイテム</div>
							<div class="search-list-title content-list-part-120">数量</div>
							<div class="search-list-title content-list-part-120">購入価格</div>
						</div>
						{foreach from=$app.accounting_log_list key="i" item="log_data"}
						...
						<div class="search-list-content-box"{if $log_data.res_flg=="1"} style="width:951px;background-color:#ffaaaa;"{/if}>
							...
							<div class="search-list-content content-list-part-150">{$log_data.user_base.name}</div>
							...
							<div>
								...
								<div class="content-monster-float-box">
									<div class="search-list-content content-list-part-100">
										{$app.item_master_list[$item.item_id].name_ja}<br />
									</div>
									<div class="search-list-content content-list-part-120">
										{$item.count}<br />
										({$item.num_prev}&nbsp;→&nbsp;<span style="font-weight:bold;">{$item.num}</span>)<br />
									</div>
								</div>
								{/foreach}
							</div>
							<div class="search-list-content content-list-part-120">{$log_data.price}</div>
						</div>
						{/foreach}
					</div>
					{/if}
