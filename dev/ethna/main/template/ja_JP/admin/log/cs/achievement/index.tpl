<!DOCTYPE html>
<html lang="ja">
	{include file="admin/common/head.tpl" title="アチーブメント履歴 - サイコパス管理ページ"}
				...
				<div class="span9 main-contents">
					...
					<div style="border: 1px solid #aaaaaa; padding:10px;margin:10px;width:600px;">
						...
						<form action="index" method="post" class="form-horizontal" id="formLogSearch">
							...
							{* <div class="search-part">
								アイテム：{form_input name="search_item_id" id="searchItemId"}<br />
							</div> *}
					...
					{elseif $app.achievement_log_count==-1}
					...
					<div style="padding:5px;">
						<div class="search-list-title-box">
							<div>
								<span>{$app.achievement_log_count}件のログが抽出されました</span>
							</div>
					...
					<div>
						<div class="search-list-title-box">
							<div class="search-list-title content-list-part-170">達成日時</div>
							<div class="search-list-title content-list-part-100">アチーブメントID</div>
							<div class="search-list-title content-list-part-170">アチーブメント名</div>
							<div class="search-list-title content-list-part-170">達成条件詳細</div>
						</div>
						{assign var=monster_list value=$app.achievement_monster_data_list}
						{foreach from=$app.achievement_log_list key="i" item="log_data"}
						<div class="search-list-content-box">
							<div class="search-list-content content-list-part-170">{$log_data.date_created}</div>
							<div class="search-list-content content-list-part-100">{$log_data.ach_id}</div>
							<div class="search-list-content content-list-part-170">{$app.achievement_master[$log_data.ach_id].name_ja}</div>
							<div class="search-list-content content-list-part-170">{$app.achievement_master[$log_data.ach_id].description_ja}</div>
						</div>
						{/foreach}
					</div>
					{/if}
