<!DOCTYPE html>
<html lang="ja">
	{include file="admin/common/head.tpl" title="エリアストレス履歴 - サイコパス管理ページ"}

	{include file="admin/log/cs/_part/log_cs_css.tpl"}

{literal}
<style>
.search-list-title-box, .search-list-content-box { clear: both; width: 920px; }
		...
		<div class="container-fluid">
			...
			<div class="row-fluid">
				...
				<div class="span9 main-contents">
					...
					<div style="border: 1px solid #aaaaaa; padding:10px;margin:10px;width:600px;">
						...
						<form action="index" method="post" class="form-horizontal" id="formLogSearch">
							...
							<div class="search-part search-part-name">
								<div>ニックネーム：{form_input name="search_name" id="searchName"}
					...
					{if $app.search_flg=="1"}
					{* 検索結果出力 *}
					{if $app.area_log_count==0}
					<div class="alert alert-warning">
						<span>対象となるログは存在しません</span>
					</div>
					{elseif $app.area_log_count==-1}
					...
					<div style="padding:5px;">
						<div class="search-list-title-box">
							<div>
								<span>{$app.area_log_count}件のログが抽出されました</span>
							</div>
					...
					<div>
						<div class="search-list-title-box">
							<div class="search-list-title content-list-part-170">変動日時</div>
							<div class="search-list-title content-list-part-150">STAGE名</div>
							<div class="search-list-title content-list-part-150">AREA名</div>
							<div class="search-list-title content-list-part-150">変動値</div>
							<div class="search-list-title content-list-part-150">変動後レベル</div>
							<div class="search-list-title content-list-part-150">サイコハザード有無</div>
						</div>
						{foreach from=$app.area_log_list key="i" item="log_data"}
						{assign var=transaction_id value=$log_data.api_transaction_id}
						<div class="search-list-content-box"{if $log_data.status=="1"} style="width:951px;background-color:#ffaaaa;"{/if}>
							<div class="search-list-content content-list-part-170">{$log_data.date_created}</div>
							<div class="search-list-content content-list-part-150">{$log_data.stage_name}</div>
							<div class="search-list-content content-list-part-150">{$log_data.area_name}</div>
							<div class="search-list-content content-list-part-150">{$log_data.area_stress-$log_data.area_stress_prev}</div>
							<div class="search-list-content content-list-part-150">{$log_data.area_stress}</div>
