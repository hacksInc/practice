<!DOCTYPE html>
<html lang="ja">
	{include file="admin/common/head.tpl" title="ガチャ履歴 - サイコパス管理ページ"}

	{include file="admin/log/cs/_part/log_cs_css.tpl"}

{literal}
<style>
.search-list-title-box, .search-list-content-box { clear: both; width: 870px; }
				...
				<div class="span9 main-contents">
					...
					<div style="border: 1px solid #aaaaaa; padding:10px;margin:10px;width:600px;">
						...
						<form action="index" method="post" class="form-horizontal" id="formLogSearch">
							...
							{* <div class="search-part"> *}
								{*    ガチャ種類：{form_input name="search_gacha_id" id="searchGachaId"}<br /> *}
								{* </div> *}
					...
					{if $app.search_flg=="1"}
					{* 検索結果出力 *}
					{if $app.gacha_log_count==0}
					<div class="alert alert-warning">
						<span>対象となるログは存在しません</span>
					</div>
					{elseif $app.gacha_log_count==-1}
					...
					<div style="padding:5px;">
						<div class="search-list-title-box">
							<div>
								<span>{$app.gacha_log_count}件のログが抽出されました</span>
							</div>
					...
					<div>
						<div class="search-list-title-box">
							...
							<div class="search-list-title content-list-part-100">ガチャID</div>
							<div class="search-list-title content-list-part-150">ガチャ名称</div>
							<div class="search-list-title content-list-part-150">ガチャ種類</div>
							<div class="search-list-title content-list-part-150">排出フォトID</div>
							<div class="search-list-title content-list-part-150">排出後フォトLv</div>
						</div>
						{foreach from=$app.gacha_log_list key="i" item="log_data"}
						<div class="search-list-content-box">
							<div class="search-list-content content-list-part-170">{$log_data.date_created}</div>
							<div class="search-list-content content-list-part-100">{$log_data.gacha_id}</div>
							<div class="search-list-content content-list-part-150">{$app.photo_gacha_list[$log_data.gacha_id].name_ja}</div>
							<div class="search-list-content content-list-part-150">{$log_data.type}</div>
