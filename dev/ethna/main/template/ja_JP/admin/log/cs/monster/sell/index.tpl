<!DOCTYPE html>
<html lang="ja">
{include file="admin/common/head.tpl" title="モンスター売却履歴 - サイコパス管理ページ"}
<body>
...
{*                <div class="search-part">
            ...
            {if $app.search_flg=="1"}
                {* 検索結果出力 *}
                {if $app.monster_sell_log_count==0}
                <div class="alert alert-warning">
                    <span>対象となるログは存在しません</span>
                </div>
                {elseif $app.monster_sell_log_count==-1}
                ...
                <div style="padding:5px;">
                    <div class="search-list-title-box">
                        <div>
                            <span>{$app.monster_sell_log_count}件のログが抽出されました</span>
                        </div>
                ...
                <div>
                    <div class="search-list-title-box">
                        ...
                        <div class="search-list-title content-list-part-150">売却情報</div>
                        <div class="search-list-title content-list-part-100">モンスターID</div>
                        <div class="search-list-title content-list-part-100">モンスター名</div>
                        ...
                        <div class="search-list-title content-list-part-50">売却価格</div>
                    </div>
                    {assign var=monster_list value=$app.monster_data_list}
                    {foreach from=$app.monster_sell_log_list key="i" item="log_data"}
                    ...
                    <div class="search-list-content-box">
                        ...
                        <div class="search-list-content content-list-part-150">
                            売却数&nbsp;:&nbsp;{$log_data.monster_cnt}<br />
                            売却金額&nbsp;:&nbsp;{$log_data.sell_price}<br />
                            ({$item_data.old_num} → {$item_data.num})
                        </div>
                        <div>
                            {foreach from=$monster_list.$transaction_id key="j" item="monster_data"}
                            <div class="content-monster-float-box">
                                <div class="search-list-content content-list-part-100">{$monster_data.user_monster_id}</div>
                                <div class="search-list-content content-list-part-100">[{$monster_data.monster_id}]{$monster_data.monster_name}<br />{$monster_data.rare}</div>
                                <div class="search-list-content content-list-part-80">{$monster_data.exp}</div>
                                <div class="search-list-content content-list-part-50">{$monster_data.lv}</div>
                                <div class="search-list-content content-list-part-50">{$monster_data.hp}</div>
                                <div class="search-list-content content-list-part-50">{$monster_data.attack}</div>
                                <div class="search-list-content content-list-part-50">{$monster_data.skill_lv}</div>
                                <div class="search-list-content content-list-part-50">{$monster_data.price}</div>
                            </div>
                            {/foreach}
                        </div>
                    </div>
