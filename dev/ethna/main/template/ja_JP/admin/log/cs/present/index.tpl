<!DOCTYPE html>
<html lang="ja">
	{include file="admin/common/head.tpl" title="プレゼント履歴 - サイコパス管理ページ"}
				...
				<div class="span9">
					...
					<div style="border: 1px solid #aaaaaa; padding:10px;margin:10px;width:600px;">
						...
						<form action="index" method="post" class="form-horizontal" id="formLogSearch">
							...
							<!--
							<div class="search-part">
								ステータス：{form_input name="search_stetus" id="searchStetus"}<br />
							</div>
							-->
					...
					{if $app.search_flg=="1"}
					{* 検索結果出力 *}
					{if $app.present_log_count==0}
					<div class="alert alert-warning">
						<span>対象となるログは存在しません</span>
					</div>
					{elseif $app.present_log_count==-1}
					...
					<div style="padding:5px;">
						<div class="search-list-title-box">
							<div>
								<span>{$app.present_log_count}件のログが抽出されました</span>
							</div>
					...
					<div>
						<div class="search-list-title-box">
							<div class="search-list-title content-list-part-170">受取日時</div>
							<div class="search-list-title content-list-part-150">配布アイテム</div>
							<div class="search-list-title content-list-part-100">配布数</div>
							<div class="search-list-title content-list-part-150">配布理由</div>
							<div class="search-list-title content-list-part-150">BOX受取</div>
						</div>
						{foreach from=$app.present_log_list key="i" item="log_data"}
						<div class="search-list-content-box">
							<div class="search-list-content content-list-part-170">{$log_data.date_created}</div>
							<div class="search-list-content content-list-part-150">{$log_data.name}</div>
							<div class="search-list-content content-list-part-100">{$log_data.num}</div>
							<div class="search-list-content content-list-part-150">{$app.present_comment[$log_data.comment_id]}</div>
							<div class="search-list-content content-list-part-150">{$app.present_status[$log_data.status]}</div>
						</div>
						{/foreach}
					</div>
					{/if}
