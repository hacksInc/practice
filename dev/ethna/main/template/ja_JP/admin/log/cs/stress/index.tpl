<!DOCTYPE html>
<html lang="ja">
	{include file="admin/common/head.tpl" title="ストレスケア履歴 - サイコパス管理ページ"}
		...
		<div class="container-fluid">
			...
			<div class="row-fluid">
				...
				<div class="span9 main-contents">
					...
					{if $app.search_flg=="1"}
					{* 検索結果出力 *}
					{if $app.stress_log_count==0}
					<div class="alert alert-warning">
						<span>対象となるログは存在しません</span>
					</div>
					{elseif $app.stress_log_count==-1}
					...
					<div style="padding:5px;">
						<div class="search-list-title-box">
							<div>
								<span>{$app.stress_log_count}件のログが抽出されました</span>
							</div>
					...
					<div>
						<div class="search-list-title-box">
							...
							<div class="search-list-title content-list-part-150">臨時ストレスケア</div>
							<div class="search-list-title content-list-part-150">セラピー診断</div>
							<div class="search-list-title content-list-part-150">定時ストレスケア</div>
							<div class="search-list-title content-list-part-150">ケア後犯罪係数</div>
						</div>
						{foreach from=$app.stress_log_list key="i" item="log_data"}
						<div class="search-list-content-box"{if $log_data.res_flg=="1"} style="width:951px;background-color:#ffaaaa;"{/if}>
							...
							<div class="search-list-content content-list-part-150">{$log_data.special_stress_care}</div>
							<div class="search-list-content content-list-part-150">{$log_data.therapy_stress_care}</div>
							<div class="search-list-content content-list-part-150">{$log_data.regular_stress_care}</div>
							<div class="search-list-content content-list-part-150">{$log_data.crime_coef}</div>
						</div>
						{/foreach}
					</div>
					{/if}
