...
if( !strpos($_SERVER['HTTP_REFERER'], "/news_confirm.php") && !strpos($_SERVER['HTTP_REFERER'], "/news_delete.php")) {
	...
	if($_SESSION["sr"] == 1) {
		$exce_kbn = "登録";
		$sql		= "INSERT INTO m_portal_news (news_kbn, disp_date, news_title, news_text, regist_user_id, update_date) VALUES (?, ?, ?, ?, ?, ?);";
		...
		$exce_kbn = "更新";
		$sql		= "UPDATE m_portal_news SET news_kbn = ?, disp_date = ?, news_title = ?, news_text = ? , regist_user_id = ?, update_date = ? WHERE id= ?;";
		$bind_param = $db->addBind($bind_param, "s", $_SESSION["kubun"]);
		$bind_param = $db->addBind($bind_param, "s", $_SESSION["date_yy"].$_SESSION["date_mm"].$_SESSION["date_dd"]);
		$bind_param = $db->addBind($bind_param, "s", $_SESSION["title"]);
		$bind_param = $db->addBind($bind_param, "s", $_SESSION["news"]);
					...
					<div class="row">
						<div class="col-lg-6">
							...
							<div class="form-group">
							 ...
							 <button type="button" class="btn btn-outline btn-success" onclick="location.href='./'">一覧に戻る</button>
						</div>
						<!-- /.col-lg-6 (nested) --> 
					</div>
					<!-- /.row (nested) --> 
