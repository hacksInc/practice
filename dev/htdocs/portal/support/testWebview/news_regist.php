...
//open_date
//年
if(!empty($open_date_yy)) {
	$open_current_y = $open_date_yy;
} else {
	$open_current_y = 0;
}
for ($i = 0; $i <= 3; $i++) {
	$open_yy = date('Y') -1 + $i;
	if($i == $open_current_y) {
		$open_yy_select .= '<option value="'.'0'.'" selected>'.'</option>';
	} else {
		$open_yy_select .= '<option value="'.$open_yy.'">'.$open_yy.'</option>';
	}
}
//月
if(!empty($open_date_mm)) {
	$open_current_m = (int)$open_date_mm;	
} else{
	$open_current_m = 0;
}
for ($i = 0; $i <= 12; $i++) {
	if($i == $open_current_m) {
		$open_mm_select .= '<option value="'.'0'.'" selected>'.'</option>';
	} else {
		$open_mm_select .= '<option value="'.sprintf("%02d", $i).'">'.$i.'</option>';
	}
}
//日
if(!empty($open_date_dd)) {
	$open_current_d = (int)$open_date_dd;	
} else{
	$open_current_d = 0;
}
$days = date('t');
for ($i = 0; $i <= $days; $i++) {
	if($i == $open_current_d) {
		$open_dd_select .= '<option value="'.'0'.'" selected>'.'</option>';
	} else {
		$open_dd_select .= '<option value="'.sprintf("%02d", $i).'">'.$i.'</option>';
	}
}
//時
if(!empty($open_date_hh)) {
	$open_current_h = (int)$open_date_hh;	
} else{
	$open_current_h = 0;
}
for ($i = 0; $i <= 23; $i++) {
...
//分
if(!empty($open_date_ii)) {
	$open_current_i = (int)$open_date_ii;	
} else{
	$open_current_i = 0;
}
for ($i = 0; $i <= 59; $i++) {
...
//秒
if(!empty($open_date_ss)) {
	$open_current_s = (int)$open_date_ss;	
} else{
	$open_current_s = 0;
}
for ($i = 0; $i <= 59; $i++) {
					...
					<div class="row">
						<div class="col-lg-6">
							<form role="form" method="post"　>
								...
								<div class="open_button"><a href="#">予約する</a></div>
								<div class="form-group day_select_group" id="open">
									<label class="fleft">公開予約日時</label>
